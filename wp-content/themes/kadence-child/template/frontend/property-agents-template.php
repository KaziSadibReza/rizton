...
?>
<div class="related-agents-container">
    <?php
        // Loop through each selected agent post
        foreach ( $related_agents as $agent_post ) {
            // Get the ID of the agent post
            $agent_id = $agent_post->ID;

            // --- Get Agent Data ---
            $agent_name = get_the_title( $agent_id );
            $agent_photo = get_the_post_thumbnail( $agent_id, 'medium' ); // we can use 'thumbnail', 'medium', 'large', or a custom size
            $agent_title = get_field('agent_department', $agent_id); // e.g., A text field for "Marketing"
            $agent_cell = get_field('phone_number', $agent_id); // Agent Phone number field
            $agent_office = get_field('office_phone_number', $agent_id); // Agent Office phone number field
            $agent_whatsapp_number = get_field('whats_app_number', $agent_id); // Agent Whats app Number field

            // Sanitize cell phone number for the WhatsApp link (removes spaces, dashes, etc.)
            $whatsapp_number = preg_replace('/[^0-9]/', '', $agent_whatsapp_number);
?>

    <div class="related-agent-card">
        <div class="agent-photo">
            <?php echo $agent_photo; ?>
        </div>
        <div class="agent-details">
            ...
            <?php if ( $agent_cell ) : ?>
            <p class="agent-cell"><strong>Cell</strong> - <?php echo esc_html( $agent_cell ); ?></p>
            <?php endif; ?>
            <?php if ( $agent_office ) : ?>
            <p class="agent-office"><strong>Office</strong> - <?php echo esc_html( $agent_office ); ?></p>
            <?php endif; ?>
